    ...
    submenu: [
      {
        ...
        click() {
          shell.openExternal(
            "https://github.com/wildfirechat/vue-pc-chat/issues"
          );
        }
      },
      {
    ...
    try {
      if (state === "interrupted") {
        ...
        if (item.isPaused()) {
          ...
          console.log(
            `Received bytes: ${fileName} ${item.getReceivedBytes()}, ${item.getTotalBytes()}`
          );
...
const createMainWindow = async () => {
  ...
  ipcMain.on(
    IPCRendererEventType.showConversationMessageHistoryPage,
    async (event, args) => {
      console.log(
        `on ${IPCRendererEventType.showConversationMessageHistoryPage}`,
        conversationMessageHistoryMessageWindow,
        args
      );
  ...
  ipcMain.on(
    IPCRendererEventType.showMessageHistoryPage,
    async (event, args) => {
      console.log(
        `on ${IPCRendererEventType.showMessageHistoryPage}`,
        messageHistoryMessageWindow,
        args
      );
